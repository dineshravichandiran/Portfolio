  ...
  return (
    <div className="container py-8 pb-16">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5 mb-14">
        {events.map((e) => (
          <TiltCard key={e.title} className="bg-panel border border-panel-border rounded-md p-6">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4 mb-14">
        {impactStats.map((s) => (
          <TiltCard key={s.label} className="bg-panel border border-panel-border rounded-md p-5.5 text-center">
      ...
      <div className="flex flex-col gap-3">
        {credentials.map((c) => (
          <TiltCard
            key={c.title}
            className="flex items-baseline gap-4 flex-wrap bg-panel border border-panel-border border-l-[3px] border-l-accent rounded-sm px-5 py-3.5"
          >
